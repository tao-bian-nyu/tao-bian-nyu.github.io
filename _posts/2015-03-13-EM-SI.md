...
Note that
<ul>
<li> if $x$ is knonw, $A$, $Q$, and $R$ can be evaluated via linear regression. 
<li> if $A$ is available, $x$ can be estimated via Kalman's filter.
<\ul>
 
\begin{itemize}
\item  if $x$ is knonw, $A$, $Q$, and $R$ can be evaluated via linear regression. 
\item if $A$ is available, $x$ can be estimated via Kalman's filter.
\end{itemize}
